import { Color, ColorList, Pixel } from "@/types/types";
import React from "react";

import { createContext, useContext, useState, useEffect } from 'react';


interface IColorListContext {
  colorList: ColorList[];
  setColorList: React.Dispatch<React.SetStateAction<ColorList[]>>;

  quantization:any
}

const ColorListContext = createContext<IColorListContext | undefined>(undefined)

export function ColorListProvider({ children }: { children: React.ReactNode }) {

  const [colorList, setColorList] = useState<ColorList[]>([])

  const buildPalette = (colorsList: Color[]) => {
    const orderedByColor = orderByLuminance(colorsList);
    let tempColors:ColorList[] = []; // Variável temporária para armazenar as cores
  ...
  return (
    <ColorListContext.Provider value={{ colorList, setColorList, quantization }}>
      {children}
    </ColorListContext.Provider>
  )
}

export const useColorListContext = () => {
  const context = useContext(ColorListContext);
  if (!context) {
    throw new Error("useColorListContext deve ser utilizado dentro de um ColorListProvider")
  }
}